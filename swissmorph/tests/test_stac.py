...
def _downloader(tmp_dir: str, cfg: Optional[dict] = None) -> StacDownloader:
    cfg = cfg or {
        "stac": {
...
# ── Deduplication of acquisition years ────────────────────────────────────────

class TestDeduplicateLatest(unittest.TestCase):

    def _item(self, item_id: str, datetime_str=None) -> dict:
        props = {"datetime": datetime_str} if datetime_str else {}
        return {"id": item_id, "properties": props}

    def test_keeps_latest_year_per_cell(self):
        """Two years of the same cell → only the most recent is kept."""
        items = [
            self._item("swissalti3d_2019_2599-1198", "2019-01-01T00:00:00Z"),
            self._item("swissalti3d_2025_2599-1198", "2025-01-01T00:00:00Z"),
        ]
        with tempfile.TemporaryDirectory() as tmp:
            kept = _downloader(tmp)._deduplicate_latest(items, lambda _: None)
        self.assertEqual(len(kept), 1)
        self.assertEqual(kept[0]["id"], "swissalti3d_2025_2599-1198")

    def test_distinct_cells_are_all_kept(self):
        """Different cells are never collapsed together."""
        items = [
            self._item("swissalti3d_2019_2599-1198", "2019-01-01T00:00:00Z"),
            self._item("swissalti3d_2019_2600-1198", "2019-01-01T00:00:00Z"),
        ]
        with tempfile.TemporaryDirectory() as tmp:
            kept = _downloader(tmp)._deduplicate_latest(items, lambda _: None)
        self.assertEqual({i["id"] for i in kept}, {
            "swissalti3d_2019_2599-1198",
            "swissalti3d_2019_2600-1198",
        })

    def test_falls_back_to_year_in_id_without_datetime(self):
        """When properties.datetime is absent, the year in the id is used."""
        items = [
            self._item("swissalti3d_2025_2599-1198"),
            self._item("swissalti3d_2019_2599-1198"),
        ]
        with tempfile.TemporaryDirectory() as tmp:
            kept = _downloader(tmp)._deduplicate_latest(items, lambda _: None)
        self.assertEqual(len(kept), 1)
...
class TestToWGS84(unittest.TestCase):
    ...
    def test_output_within_switzerland(self):
        ...
        # Approximate centre of Switzerland in LV95 (EPSG:2056).
        # LV95 eastings are ~2.4–2.85 M, northings ~1.07–1.30 M; the old
        # LV03 values (6xx xxx / 1xx xxx) are *not* valid EPSG:2056 input.
        bbox_lv95 = (2620000.0, 1180000.0, 2780000.0, 1270000.0)
        with tempfile.TemporaryDirectory() as tmp:
            lon_min, lat_min, lon_max, lat_max = _downloader(tmp)._to_wgs84(bbox_lv95)

        # Switzerland bounding box in WGS84
        self.assertGreater(lon_min, 5.9,  "lon_min must be east of 5.9°")
        self.assertLess(lon_max,    10.6,  "lon_max must be west of 10.6°")
        self.assertGreater(lat_min, 45.8,  "lat_min must be north of 45.8°")
        self.assertLess(lat_max,    47.9,  "lat_max must be south of 47.9°")
    ...
    def test_small_bbox_stays_small(self):
        """A 1 km² bbox in LV95 must map to < 0.02° in WGS84."""
        try:
            from pyproj import Transformer
        except ImportError:
            self.skipTest("pyproj not available")

        bbox_lv95 = (700000.0, 200000.0, 701000.0, 201000.0)  # 1 km²
        with tempfile.TemporaryDirectory() as tmp:
            lon_min, lat_min, lon_max, lat_max = _downloader(tmp)._to_wgs84(bbox_lv95)
...
class TestQueryItemsPagination(unittest.TestCase):
    ...
    def _page(self, n_items: int, next_url: Optional[str] = None) -> dict:
